...
pub fn day_4() {
    ...
    let dirs_to_check = |(row, col): (usize, usize), (max_row, max_col): (usize, usize)| {
        ...
        (-1..=1)
            .flat_map(|i: isize| {
                (-1..=1).filter_map(move |j: isize| {
                    let is_valid = (i != 0 || j != 0)
                        && row + (i * 3) >= 0
                        && col + (j * 3) >= 0
                        && row + (i * 3) < (max_row as isize)
                        && col + (j * 3) < (max_col as isize);

                    is_valid.then(|| {
                        (0..=3)
                            .map(|d| ((row + i * d) as usize, (col + j * d) as usize))
                            .collect::<Vec<_>>()
                    })
                })
            })
