...
pub fn day_5() {
    ...
    let sum_of_valid_middle = pages_to_produce
        .clone()
        .into_iter()
        .filter(|pages| {
            pages
                ...
                .all(|(first, second)| !ordering_rules.contains(&(second, first)))
        })
        .map(|pages| {
    ...
    let sum_of_corrected_middle = pages_to_produce
        .iter()
        .filter(|pages| {
            !pages
                .iter()
